...
public class Tipo_usuarioDao implements CRUD<Tipo_usuarioDto>
{
        ...
        private static final String BUSCAR  = "SELECT * FROM tipo_usuario WHERE id_tusuario=?";
    ...
    public boolean create(Tipo_usuarioDto D) 
    {
        PreparedStatement stat;
            try {
                ...
                stat.setString(2, D.getT_usuario());
                ...
                try 
                    {
                     ...
                     res = stat.executeQuery();
                        while (res.next())
                        {
                          t = new Tipo_usuarioDto(res.getInt(1),res.getString(2));
                        }
                    } catch (SQLException ex) 
                        {
                         Logger.getLogger(Tipo_usuarioDao.class.getName()).log(Level.SEVERE, null, ex);
            ...
            try 
                {
                 ...
                 stat.setString(1, D.getT_usuario());
            ...
            try 
                {
                 ...
                 res = stat.executeQuery();
                   while (res.next()) 
                       {
                        tipo_usuario.add(new Tipo_usuarioDto(res.getInt(1),res.getString(2)));
                       }
                } catch (SQLException ex) 
                   {
                    Logger.getLogger(Tipo_usuarioDao.class.getName()).log(Level.SEVERE, null, ex);
